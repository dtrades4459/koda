...
function AuthForm({ onSuccess, initialError = "" }: { onSuccess: () => void; initialError?: string }) {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      if (e) throw e;
      // Redirect happens — onSuccess fires via auth state change in TradrAuth
    } catch (e: any) {
      setError(e?.message || "Google sign-in failed. Try username/password instead.");
      setLoading(false);
    }
  }
    ...
    <div style={{ padding: "28px 0 8px", borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}` }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px", margin: "4px 0" }}>
          ...
          <span style={{ fontFamily: MONO, fontSize: "9px", color: C.dim, letterSpacing: "0.1em" }}>OR</span>
          <div style={{ flex: 1, height: "1px", background: C.border }} />
        </div>

        <button onClick={handleGoogle} disabled={loading}
          style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "10px",
            background: "transparent", border: `1px solid ${C.border2}`, borderRadius: "10px",
            padding: "12px 16px", cursor: "pointer", width: "100%", color: C.text,
            fontFamily: BODY, fontSize: "14px", transition: "border-color 0.15s" }}
